...
#[OA\Get(
    ...
    parameters: [
        new OA\PathParameter(
            name: 'secret',
            description: 'Secret token issued to allow the password reset. ' .
                'Use this to check if a token is valid before inviting user to enter their new password',
            schema: new OA\Schema(type: 'string', example: 'EivZrmdxk4YJXQC37Q6Cnu', pattern: '[a-zA-Z0-9]{5-60}'),
        ),
    ],
    responses: [
        new OA\Response(response: 200, description: 'Secret token found'),
        new OA\Response(response: 404, description: 'Secret token not found - may have never existed or expired'),
    ],
)]
