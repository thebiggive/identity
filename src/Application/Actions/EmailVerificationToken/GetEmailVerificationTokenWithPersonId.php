...
/**
 * @see SetFirstPassword
 */
#[OA\Get(
    path: '/v1/emailVerificationToken/{secret}/{personId}',
    summary: 'Get an email verification token to check if its valid for an existing user account ' .
        'that does not yet have a password',
    parameters: [
    ...
    responses: [
        new OA\Response(
            response: 200,
            description: 'Token created and emailed to user, if they are NOT already registered, ' .
                'which the server does not confirm or deny via http. ' .
                'If they are already registered then they are sent a message to remind them of that.',
